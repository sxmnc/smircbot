    ...
    setInterval(function(){
        if (playing && !playPhase) {
            ...
            if (timer == 15) {
                ...
                bot.sayPub('game is starting! joinning closed, get ready to play!');
                joinPhase = false;
                playPhase = true;
                timer = 0;
            }
    ...
    var listener = function (nick, text, msg) {
        ...
        if (text.indexOf(command) === 0) {
            ...
            if (arg == 'start'){
                ...
                if(!playing){
                    ...
                    bot.sayPub(nick + ': its too late to join!');
                } else {
        ...
        if(writePhase){
            if(text.indexOf(phrase) === 0 && players.indexOf(nick) !== -1){
                bot.sayPub(nick + ' as wrote the phrase faster!');
                writePhase = false;
                timer = 0;
            } else if(players.indexOf(nick) !== -1) {
                bot.sayPub(nick + ' you are wrong! ' + nick + ' is no longer in the game!');
